...
**ex) `subsequences("abc")` → `“abc,ab,bc,ac,a,b,c,”`**
...
**메소드의 동작은 프로그램의 다른 부분에 `side-effect`가 없는 매개 변수와 반환 값 간의 관계로 간단히 이해될 수 있다. 이러한 종류의 패러다임을 `functional programming`(함수형 프로그래밍)이라고 하며, 루프와 변수를 사용한 명령형 프로그래밍보다 추론하기 훨씬 쉽다.**
