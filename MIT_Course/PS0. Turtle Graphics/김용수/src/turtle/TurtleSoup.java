...
public class TurtleSoup {
    ...
    public static double calculateRegularPolygonAngle(int sides) {
        if (sides <= 2) {
            throw new IllegalArgumentException("sides <= 2");
        }
    ...
    public static int calculatePolygonSidesFromAngle(double angle) {
        if (angle <= 0 || angle >= 180)
            throw new IllegalArgumentException("0 <= angle <= 180 is error");

        double sumOfexternal = 180 - angle;
        double sides = Math.ceil(360 / sumOfexternal);
        return (int) sides;
    }
    ...
    public static double calculateHeadingToPoint(double currentHeading, int currentX, int currentY,
        ...
        final double BaseAngle = 90;

        double radian = Math.atan2(targetY - currentY, targetX - currentX) * 180 / Math.PI;
        double degree = BaseAngle - currentHeading - radian;
    ...
    public static List<Double> calculateHeadings(List<Integer> xCoords, List<Integer> yCoords) {
        ...
        for (int i = 1; i < xCoords.size(); i++) {
            int prevX = xCoords.get(i - 1);
            int prevY = yCoords.get(i - 1);
            int curX = xCoords.get(i);
            int curY = yCoords.get(i);
            currentHeading = calculateHeadingToPoint(currentHeading, prevX, prevY, curX, curY);
